...
export class Decoder {
  ...
  public async decodeBlackbox(blackbox: ArrayBuffer) {
    ...
    const { exists: logFileExists } =
      await this.BlackboxDecodeModule.FS.analyzePath("/logs/logfile.bbl");
    if (logFileExists) {
      await this.BlackboxDecodeModule.FS.unlink("/logs/logfile.bbl");
    }

    const { exists: logsDirExists } =
      await this.BlackboxDecodeModule.FS.analyzePath("/logs");
    if (!logsDirExists) {
    ...
    const csvFiles: CSVFile = csvFileNames.map(
      (fileName: string) => {
