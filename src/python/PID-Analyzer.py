...
def run(data, headdict):
    analyzer = PIDAnalyzer(data, headdict)
    [roll, pitch, yaw], headdict = analyzer.analyze()

    with open('response_yaw.json', 'w', encoding='utf-8') as f:
        f.write(yaw.to_json())
    f.close()

    with open('response_roll.json', 'w', encoding='utf-8') as f:
        f.write(roll.to_json())
    f.close()

    with open('response_pitch.json', 'w', encoding='utf-8') as f:
        f.write(pitch.to_json())
    f.close()

    with open('headdict.json', 'w', encoding='utf-8') as f:
